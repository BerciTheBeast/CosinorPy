...
def population_fit_group(df, n_components = 2, period = 24, folder = '', prefix='', names = [], return_individual_params = False, **kwargs):
            ...
            for per in period:            
                ...
                if folder:                    
                    ...
                    if return_individual_params:
                        _, statistics, statistics_params, rhythm_params, _, ind_params = population_fit(df_pop, n_components = n_comps, period = per, save_to = save_to, return_individual_params = return_individual_params, **kwargs)
                    else:
                        _, statistics, statistics_params, rhythm_params, _ = population_fit(df_pop, n_components = n_comps, period = per, save_to = save_to, return_individual_params = return_individual_params, **kwargs)
                else:                    
                    if return_individual_params:
                        _, statistics, statistics_params, rhythm_params, _, ind_params = population_fit(df_pop, n_components = n_comps, period = per, return_individual_params = return_individual_params, **kwargs)
                    else:
                        _, statistics, statistics_params, rhythm_params, _ = population_fit(df_pop, n_components = n_comps, period = per, return_individual_params = return_individual_params, **kwargs)
...
# rhythm params
def evaluate_rhythm_params(X,Y, project_acrophase=True, period=0, phase=None):
    #plt.plot(X,Y)
    #plt.show()    
    ...
    if len(locs) >= 1 and phase is None:
       PHASE = X[locs[0]]
       PHASE_LOC = locs[0]
    ...
    # assumminig the period is good, split into chunks of length period and compute stats for each chunk spearately
    periodsX = []
    periodsY = []

    if (0 != PHASE_LOC):
        periodsX.append(X[0:PHASE_LOC])
        periodsY.append(Y[0:PHASE_LOC])
    ...
    while True:
        ...
        if (len(endLocs) > 0 and len(endLocs[0]) > 0):
            endLoc = endLocs[0][0]
            periodsX.append(X[startLoc:endLoc])
            periodsY.append(Y[startLoc:endLoc])
            startLoc = endLoc
        else:
            if (startLoc != len(X)):
                periodsX.append(X[startLoc:len(X)])
                periodsY.append(Y[startLoc:len(X)])
            break
        
    stats = []

    for i in range(0, len(periodsX)):
        stats.append(get_interval_stats(periodsY[i], mean, std))
    ...
    if len(stats) > 0:
        ...
        for key in keys:
            ...
            for i in range(len(stats)):
                statsDict[key].append(stats[i][key])
    ...
    # rhythm_params
    return {'period':period, 'amplitude':AMPLITUDE, 'acrophase':ACROPHASE, 'mesor':MESOR, 'peaks': peaks, 'heights': heights, 'troughs': troughs, 'heights2': heights2, 'max_loc': PHASE_LOC, 'period2':period2, 'interval_stats': stats, 'interval_stats_obj': statsDict, 'std': std, 'mean': mean, 'z_scores': z_scores }

def get_interval_stats(Y, total_mean, total_stdev):
    m_per = min(Y)
    M_per = max(Y)
    A_per = M_per - m_per
    MESOR_per = m_per + A_per/2
    AMPLITUDE_per = abs(A_per/2)
    std = np.std(Y)
    ...
    mean = np.mean(Y)
    stderr = stats.sem(Y)
    z_scores_total = (total_mean - Y) / total_stdev
    z_scores = (mean - Y) / std

    return {'m_per': m_per, 'M_per': M_per, 'A_per': A_per, 'MESOR_per': MESOR_per, 'AMPLITUDE_per': AMPLITUDE_per, 'std': std, 'skewness': skewness, 'kurtosis': kurtosis, 'median': median, 'mean': mean, 'stderr': stderr, 'z_scores': z_scores, 'z_scores_total': z_scores_total}
...
def compare_pair_df_extended(df, test1, test2, n_components = 3, period = 24, n_components2 = None, period2 = None, lin_comp = False, model_type = 'lin', alpha = 0, save_to = '', non_rhythmic = False, plot=True, plot_measurements=True, plot_residuals=False, plot_margins=True, x_label = '', y_label = '', additional_analysis = "", bootstrap_type="std", bootstrap_size=1000, samples_per_param_CI=5, max_samples_CI = 1000, sampling_type="LHS", parameters_to_analyse = ['amplitude', 'acrophase', 'mesor'], parameters_angular = ['acrophase']):
    ...
    amps = []
    for i in range(0, len(rhythm_params1['interval_stats'])):
        amps.append(rhythm_params1['interval_stats'][i]['A_per'])
...
def compare_models(RSS1, RSS2, DF1, DF2):
    # to hopefully prevent division by 0
    small_number = 1e-10
    if abs(RSS1) < small_number:
        RSS1 += (math.copysign(1, RSS1) * small_number)
    if abs(RSS2) < small_number:
        RSS2 += (math.copysign(1, RSS2) * small_number)
    if abs(DF1) < small_number:
        DF1 += (math.copysign(1, DF1) * small_number)
    if abs(DF2) < small_number:
        DF2 += (math.copysign(1, DF2) * small_number)
